...
main()
{
	...
	double cu_31[2][2][24][36],cd_32[2][2][24][36],cu_42[2][2][36][24],cd_42[2][2][36][24];
	...
	for(x=0;x<=1;++x){
		for(y=0;y<=1;++y){
			for(i=0;i<=36;++i){
				for(j=0;j<=24;++j){
					cu_42[x][y][i][j]=cd_42[x][y][i][j]=cu_53[x][y][j][i]=cd_52[x][y][j][i]=0.0;
				};
			};
		};
	};
	// d [3,1](0,.,4,.,8,.,12,.,16,.,20,...,23) [4,2](0,.,6,.,12,.,18,.,24,.,30,...,35)
	cd_31[0][0][0][0]=cd_31[0][0][2][1]=cd_31[0][0][7][2]=-1.0;
	cd_31[0][0][4][6]=cd_31[0][0][6][24]=cd_31[0][0][11][25]=-1.0;
	cd_31[0][0][8][12]=cd_31[0][0][10][30]=cd_31[0][0][15][31]=-1.0;
	cd_31[0][0][12][18]=cd_31[0][0][14][36]=cd_31[0][0][19][37]=1.0;
	cd_31[0][0][16][24]=cd_31[0][0][18][42]=cd_31[0][0][23][43]=1.0;
	cd_31[0][0][20][30]=cd_31[0][0][22][48]=cd_31[0][0][27][49]=1.0;
	...
	cout << "AF="; cin >> AF;
	// tt のループ、始まり
	for(n_tt=0;n_tt<=40;++n_tt){
		tt=-2.0+0.1*n_tt;
		if(fabs(tt-t)<0.0000001) tt=t;
		if(fabs(tt)<0.00001) tt=0.00001;
	cout<<"-----(tt="<<tt<<")-------\n";
	...
	*/
	...
	// omega のループ、始まり
	for(n_omega=0;n_omega<=400;++n_omega){
	 omega=-20.0+n_omega/20.0;
	 ...
	 for(i=0;i<=3;++i){
	 	if(gRu[i]>0.0){
	 		Sur[n_tt]-=1.0/(4.0*20.0); Nur+=1;
	 	};
	 	if(gRd[i]>0.0){
	 		Sdr[n_tt]-=1.0/(4.0*20.0); Ndr+=1;
	 	};
	 };
	 	...
	 	for(ky=0;ky<=128;++ky){
			Kx=pi*(kx-63.0)/64.0;
	 		...
	 		// 元の系のグリーン関数の逆行列の対角化
	 		 ...
	 		 for(i=0;i<=3;++i){
	 		 	if(gIu[i]>0.0){
	 		 		Sug[n_tt]-=1.0/(4.0*128.0*128.0*20.0); Nug+=1;
	 		 	};
	 		 	if(gId[i]>0.0){
	 		 		Sdg[n_tt]-=1.0/(4.0*128.0*128.0*20.0); Ndg+=1;
	 		 	};
	 		 };
	 	};
	 };
	...
	// 熱力学ポテンシャル
	Omega_t[n_tt]=Omega_r[n_tt]-Sur[n_tt]-Sdr[n_tt]+Sug[n_tt]+Sdg[n_tt];
	Sr[n_tt]=Sur[n_tt]+Sdr[n_tt]; Sg[n_tt]=Sug[n_tt]+Sdg[n_tt];
	cout <<"Omega_t="<<Omega_t[n_tt]<<"\n";
	cout <<"(Omega_r,Sr,Sg)=("<<Omega_r[n_tt]<<","<<Sr[n_tt]<<","<<Sg[n_tt]<<")\n";
	cout <<"(Sur,Sdr;Sug,Sdg)=("<<Sur[n_tt]<<","<<Sdr[n_tt]<<";"<<Sug[n_tt]<<","<<Sdg[n_tt]<<")\n";
	cout <<"(Nur,Ndr;Nug,Ndg)=("<<Nur<<","<<Ndr<<";"<<Nug<<","<<Ndg<<")\n";
	
	if(Omega>Omega_t[n_tt]){
		Omega=Omega_t[n_tt]; tta=tt;
	};
	...
	fout.open("Omega_t.dat");
	for(n_tt=0;n_tt<=40;++n_tt){
		tt=-2.0+0.1*n_tt;
		if(fabs(tt)<0.00001) tt=0.00001;
		fout << tt << " " << Omega_t[n_tt] << "\n";
	};
	fout.close();
	
	fout.open("Omega_r.dat");
	for(n_tt=0;n_tt<=40;++n_tt){
		tt=-2.0+0.1*n_tt;
		if(fabs(tt)<0.00001) tt=0.00001;
		fout << tt << " " << Omega_r[n_tt] << "\n";
	};
	fout.close();
	...
	/*
	fout.open("Sur.dat");
	for(n_tt=0;n_tt<=10;++n_tt){
		fout << Sur[n_tt] << "\n";
	};
	fout.close();
	
	fout.open("Sdr_tt_12_6_s.dat");
	for(n_tt=0;n_tt<=10;++n_tt){
		fout << Sdr[n_tt] << "\n";
	};
	fout.close();
	
	fout.open("Sug_tt_12_6_s.dat");
	for(n_tt=0;n_tt<=10;++n_tt){
		fout << Sug[n_tt] << "\n";
	};
	fout.close();
	
	fout.open("Sdg_tt_12_6_s.dat");
	for(n_tt=0;n_tt<=10;++n_tt){
		fout << Sdg[n_tt] << "\n";
	};
	fout.close();
