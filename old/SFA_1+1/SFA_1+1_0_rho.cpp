...
main()
{
	...
	int i,j,m,n,n_omega,q;
	int x,xx,k,kk; double K,KK;
	const double pi=3.1415926535897932; const double delta=0.1;
	// 生成消滅演算子
	...
	double omega,Omega_r,Omega_t,Omega,Sur,Sdr,Sug,Sdg,Sr,Sg,N,Nu,Nd;
	...
	double gui,gdi,rhou[200000],rhod[200000];
	// 元の系のパラメーター
	t=1.0;
	cout << "U="; cin >> U;
	mu=U/2.0; //cout << "mu="; cin >> mu;
	h=0.0; //cout << "h="; cin >> h;
	// 代理系のパラメーター
	cout << "V="; cin >> V;
	...
	// 初期化
	Omega_t=Omega_r=Sur=Sdr=Sug=Sdg=Nu=Nd=0.0;
	for(i=0;i<=5;++i){
		for(j=0;j<=5;++j){
			H0[i][j]=0.0;
		};
	...
	for(n=0;n<=199999;++n){
	 ...
	 for(k=0;k<=63;++k){
	 	...
	 	rhou[n]+=gui/(64.0*pi); rhod[n]+=gdi/(64.0*pi);
	 	Nu+=rhou[n]*0.0001/64.0; Nd+=rhod[n]*0.0001/64.0;
	 };
	};// omega のループ、終わり
	
	// データの書き込み
	ofstream fout;
	fout.open("SFA_1+1_0_rhou.dat");
	for(n=0;n<=199999;++n){
		fout << rhou[n] << "\n";
	};
	fout.close();
	
	fout.open("SFA_1+1_0_rhod.dat");
	for(n=0;n<=199999;++n){
		fout << rhod[n] << "\n";
	};
	fout.close();
