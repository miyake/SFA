...
main()
{
	int i,j,n,m,N,n_omega,x,xx,y,yy,kx,kkx,ky,kky; int n_P;
	...
	double e,t,tn,U,mu,h; double ee,tt,ttn,F,AF,P; double alpha; double AFa;
	...
	double omega,Omega_r,Omega_t[31],Omega,Sur,Sdr,Sug,Sdg,Sr,Sg;
	int Nur,Ndr,Nug,Ndg;
	...
	// 代理系のパラメーター
	ee=0.0; tt=1.0; ttn=0.0; F=0.0; P=0.0;
	// AF のループ、始まり
	for(n_P=0;n_P<=30;++n_P){
		P=-1.5+0.1*n_P;
		if(fabs(P)<0.0000001) P=0.0;
	cout<<"-----(P="<<P<<")-------\n";
	...
	// 熱力学ポテンシャルなどの初期化
	Omega_t[n_P]=0.0; Omega_r=Sr=Sg=0.0;
	...
	*/
	...
	// omega のループ、始まり
	for(n_omega=0;n_omega<=200;++n_omega){
	 omega=-20.0+n_omega/10.0;
	 ...
	 for(i=0;i<=3;++i){
	 	if(gRu[i]>0.0){
	 		Sur-=1.0/(4.0*10.0); Nur+=1;
	 	};
	 	if(gRd[i]>0.0){
	 		Sdr-=1.0/(4.0*10.0); Ndr+=1;
	 	};
	 };
	 	...
	 	for(ky=0;ky<=63;++ky){
			Kx=pi*(kx-31.0)/32.0;
	 		...
	 		// 元の系のグリーン関数の逆行列の対角化
	 		 ...
	 		 for(i=0;i<=3;++i){
	 		 	if(gIu[i]>0.0){
	 		 		Sug-=1.0/(4.0*64.0*64.0*10.0); Nug+=1;
	 		 	};
	 		 	if(gId[i]>0.0){
	 		 		Sdg-=1.0/(4.0*64.0*64.0*10.0); Ndg+=1;
	 		 	};
	 		 };
	 	};
	 };
	...
	fout.open("Omega_t_P_16_8_s.dat");
	for(n_P=0;n_P<=30;++n_P){
		P=-1.5+0.1*n_P;
		fout << P << " " << Omega_t[n_P]-Omega_t[15] << "\n";
	};
	fout.close();
	/*
