...
main()
{
	int i,j,n,m,N,n_omega,x,xx,y,yy,kx,kkx,ky,kky; int n_U;
	...
	double omega,Omega_r,Omega_t[20],Omega,Sur,Sdr,Sug,Sdg,Sr,Sg;
	int Nur,Ndr,Nug,Ndg;
	...
	// 熱力学ポテンシャルなどの初期化
	Omega_r=0.0; Omega_t[n_U]=0.0; Sr=Sg=0.0;
	...
	*/
	...
	// omega のループ、始まり
	for(n_omega=0;n_omega<=2000;++n_omega){
	 omega=-20.0+n_omega/100.0;
	 ...
	 for(i=0;i<=3;++i){
	 	if(gRu[i]>0.0){
	 		Sur-=1.0/(4.0*100.0); Nur+=1;
	 	};
	 	if(gRd[i]>0.0){
	 		Sdr-=1.0/(4.0*100.0); Ndr+=1;
	 	};
	 };
	 ...
	 for(kx=0;kx<=31;++kx){
	 	for(ky=0;ky<=31;++ky){
			Kx=pi*(kx-15.0)/16.0;
	 		...
	 		GIu[1][0]=t-SEu[1][0];  GIu[2][0]=t-SEu[2][0];
	 		GIu[3][1]=t-SEu[3][1];  GIu[3][2]=t-SEu[3][2];
	 		GIu[3][0]=tn-SEu[3][0]; GIu[2][1]=tn-SEu[2][1];
	 		GId[1][0]=t-SEd[1][0];  GId[2][0]=t-SEd[2][0];
	 		GId[3][1]=t-SEd[3][1];  GId[3][2]=t-SEd[3][2];
	 		GId[3][0]=tn-SEd[3][0]; GId[2][1]=tn-SEd[2][1];
	 		// 元の系のグリーン関数の逆行列の対角化
	 		 ...
	 		 for(i=0;i<=3;++i){
	 		 	if(gIu[i]>0.0){
	 		 		Sug-=1.0/(4.0*1024.0*100.0); Nug+=1;
	 		 	};
	 		 	if(gId[i]>0.0){
	 		 		Sdg-=1.0/(4.0*1024.0*100.0); Ndg+=1;
	 		 	};
	 		 };
	 	};
	 };
	...
	// 熱力学ポテンシャル
	Omega_t[n_U]=Omega_r-Sur-Sdr+Sug+Sdg; Sr=Sur+Sdr; Sg=Sug+Sdg;
	cout <<"Omega_t="<<Omega_t[n_U]<<"\n";
	...
	fout.open("Omega_hf_U_2x2_s.dat");
	for(n_U=0;n_U<=19;++n_U){
		U=1.0+1.0*n_U;
		fout << U << " " << Omega_t[n_U] << "\n";
	};
	...
	fout.open("E_hf_U_2x2_s.dat");
	for(n_U=0;n_U<=19;++n_U){
		U=1.0+1.0*n_U;
		fout << U << " " << Omega_t[n_U]+U/2.0 << "\n";
	};
	fout.close();
