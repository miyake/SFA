...
main()
{
	int i,j,n,m,N,n_omega,x,xx,y,yy,kx,kkx,ky,kky; int n_ttn;
	...
	double e,t,tn,U,mu,h; double ee,tt,ttn,F,AF,Px,Py; double alpha; double ttna;
	...
	double omega,Omega_r[11],Omega_t[11],Omega,Sur[11],Sdr[11],Sug[11],Sdg[11],Sr[11],Sg[11];
	int Nur,Ndr,Nug,Ndg;
	...
	// 代理系のパラメーター
	ee=0.0; tt=t; F=0.0; Px=0.0; Py=0.0;
	cout << "AF="; cin >> AF;
	// tt のループ、始まり
	for(n_ttn=0;n_ttn<=10;++n_ttn){
		ttn=tn-0.5+0.1*n_ttn;
		if(fabs(ttn-tn)<0.0000001) ttn=tn;
	cout<<"-----(ttn="<<ttn<<")-------\n";
	...
	// 熱力学ポテンシャルなどの初期化
	Omega_r[n_ttn]=Omega_t[n_ttn]=Sr[n_ttn]=Sg[n_ttn]=0.0;
	Sur[n_ttn]=Sdr[n_ttn]=Sug[n_ttn]=Sdg[n_ttn]=0.0; Nur=Ndr=Nug=Ndg=0;
	// 代理系のハミルトニアンの対角成分
	// [0,0] 1 (0:0)
	H0[0][0]=0.0;
	...
	*/
	...
	for(n_omega=0;n_omega<=5000;++n_omega){
	 ...
	 for(i=0;i<=3;++i){
	 	if(gRu[i]>0.0){
	 		Sur[n_ttn]-=1.0/(4.0*250.0); Nur+=1;
	 	};
	 	if(gRd[i]>0.0){
	 		Sdr[n_ttn]-=1.0/(4.0*250.0); Ndr+=1;
	 	};
	 };
	 ...
	 for(i=0;i<=3;++i){
	 	for(j=0;j<=3;++j){
	 		...
	 		if(i==j){
				SEu[i][j]-=1.0/gRu[i]; SEd[i][j]-=1.0/gRd[i];
			};
			/*
	 		if(i!=j){
				if(fabs(SEu[i][j])>0.00001){
					cout<<"(omega;i,j)=("<<omega<<";"<<i<<","<<j<<"), SEu[i][j]="<<SEu[i][j]<<"\n";
				};
				if(fabs(SEd[i][j])>0.00001){
					cout<<"(omega;i,j)=("<<omega<<";"<<i<<","<<j<<"), SEd[i][j]="<<SEd[i][j]<<"\n";
				};
			};
			*/
	 	};
	 	...
	 	for(ky=0;ky<=31;++ky){
			Kx=pi*(kx-15.0)/16.0;
	 		...
	 		// 元の系のグリーン関数の逆行列の対角化
	 		 ...
	 		 for(i=0;i<=3;++i){
	 		 	if(gIu[i]>0.0){
	 		 		Sug[n_ttn]-=1.0/(4.0*1024.0*100.0); Nug+=1;
	 		 	};
	 		 	if(gId[i]>0.0){
	 		 		Sdg[n_ttn]-=1.0/(4.0*1024.0*100.0); Ndg+=1;
	 		 	};
	 		 };
	 	};
	 };
	...
	// 熱力学ポテンシャル
	Omega_t[n_ttn]=Omega_r[n_ttn]-Sur[n_ttn]-Sdr[n_ttn]+Sug[n_ttn]+Sdg[n_ttn];
	Sr[n_ttn]=Sur[n_ttn]+Sdr[n_ttn]; Sg[n_ttn]=Sug[n_ttn]+Sdg[n_ttn];
	cout <<"Omega_t="<<Omega_t[n_ttn]<<"\n";
	cout <<"(Omega_r,Sr,Sg)=("<<Omega_r[n_ttn]<<","<<Sr[n_ttn]<<","<<Sg[n_ttn]<<")\n";
	cout <<"(Sur,Sdr;Sug,Sdg)=("<<Sur[n_ttn]<<","<<Sdr[n_ttn]<<";"<<Sug[n_ttn]<<","<<Sdg[n_ttn]<<")\n";
	cout <<"(Nur,Ndr;Nug,Ndg)=("<<Nur<<","<<Ndr<<";"<<Nug<<","<<Ndg<<")\n";
	
	if(Omega>Omega_t[n_ttn]){
		Omega=Omega_t[n_ttn]; ttna=ttn;
	};
	...
	fout.open("Omega_t_ttn_12_6_s.dat");
	for(n_ttn=0;n_ttn<=10;++n_ttn){
		ttn=tn-0.5+0.1*n_ttn;
		fout << ttn << " " << Omega_t[n_ttn] << "\n";
	};
	fout.close();
	
	fout.open("Omega_r_ttn_12_6_s.dat");
	for(n_ttn=0;n_ttn<=10;++n_ttn){
		ttn=tn-0.5+0.1*n_ttn;
		fout << ttn << " " << Omega_r[n_ttn] << "\n";
	};
	fout.close();
	
	fout.open("Sr_ttn_12_6_s.dat");
	for(n_ttn=0;n_ttn<=10;++n_ttn){
		ttn=tn-0.5+0.1*n_ttn;
		fout << ttn << " " << Sr[n_ttn] << "\n";
	};
